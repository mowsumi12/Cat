...
module.exports.run = async function ({ api: b, event: a, args: c }) {
    ...
    if (0 == c.length) return f("𝐇𝐦𝐦. 𝐛𝐨𝐥𝐨 𝐬𝐨𝐧𝐚😘😘");
    switch (c[0]) {
        case "on":
            return global.simsimi.has(d) ? f("You have not turned off the sim.") : (global.simsimi.set(d, e), f("Bolo janu ami aschi."));
        case "off":
            return global.simsimi.has(d) ? (global.simsimi.delete(d), f("janu bye bye.")) : f("You have not enabled the sim.");
        default:
